		...
		<div class="row-fluid">
			...
			<div class="span4">
				<div class="control-group pull-right">
					<label class="control-label" for="InvoiceType">Request Type</label>
   		...
   		<form action="#" class="form-horizontal invoice-add-form">
					...
					<div class="containerCustomerForRegularSale">
						<div class="control-group">
							<div class="input-append">
								<input type="text" class="input-medium customer-name" placeholder="Enter requester name" />
								<button class="btn btn-check-customer-byname" type="button">Search</button>
							</div>
						</div>

						<!-- <div class="control-group">
							<div class="input-append">
								<input type="text" class="input-medium customer-contact" placeholder="Enter requester mobile" />
								<button class="btn btn-check-customer" type="button">Search!</button>
							</div>
						</div> -->

						<div class="pager msg-for-empty-customer"><strong>No requester added yet.</strong></div>
						<div id="placeholder-customer"></div>
						<a class="btn btn-success" data-target="#ModalCustomerAdd" data-toggle="modal">
							<i class="icon-plus icon-white"></i>
							<span>Add new requester</span>
						</a>
					</div>
					<div class="containerCustomerForCashSale hide">
						<div class="control-group">
					        <label class="control-label" for="CustomerNameForCashSale">Requester Name</label>
					        <div class="controls">
		    ...
		    <div class="modal-footer">
        		...
        		<button type="button" class="btn-add-invoice btn btn-success btn-ajax">
            		<i class="icon-shopping-cart icon-large icon-white"></i> Create Request
        		</button>
...
<script id="template-customer" type="text/x-handlebars-template">
	{{#if customer}}
		...
		{{/customer}}
	{{else}}
		<p><strong class="text-error">Invalid requester contact. Please try again or add a new requester.</strong></p>
	{{/if}}
</script>
...
<script>
	$(function () {
		...
		*/
		...
		// ini untuk render by Name

		function renderCustomerDetailsbyName(form_data) {
			$('.msg-for-empty-customer').hide();
			$.fetchRemoteData('customer', 'getCustomerByName', form_data).then(function (data) {
				renderHandlebarsTemplate('#template-customer', '#placeholder-customer', data);
				$.hideSpinner();
			});
		}
		$(document).on('click', '.btn-check-customer-byname', function () {
			/* retrieve customer information if any */
			var form_data = {
				CustomerName: $('.customer-name').val()
			};
			renderCustomerDetailsbyName(form_data);
		});
		...
		$(document).on('change', '.InvoiceType', function () {
			...
			if(invoiceType.toLowerCase() === 'regular request') {
